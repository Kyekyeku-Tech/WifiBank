      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-8 flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-300">Manage credentials & track transactions</p>
          </div>
          <button
        ...
        <section className="bg-slate-800 rounded-2xl p-6 mb-8 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            <input placeholder="Username" value={username} onChange={e => setUsername(e.target.value)} className="p-2 rounded bg-white/10"/>
            <input placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="p-2 rounded bg-white/10"/>
            <select value={packageId} onChange={e => setPackageId(e.target.value)} className="p-2 rounded bg-black/10">
              {packages.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <input placeholder="Description (optional)" value={description} onChange={e => setDescription(e.target.value)} className="p-2 rounded bg-white/10"/>
          </div>
          <button onClick={addCredential} disabled={processing} className="mt-4 bg-sky-600 px-4 py-2 rounded-full hover:bg-sky-500">
            Add Credential
          </button>
        </section>
        ...
        <section className="mb-8">
          ...
          {loadingCreds ? <p>Loading credentials...</p> :
            <div className="overflow-x-auto bg-slate-800 rounded-2xl p-4 shadow">
              <table className="w-full table-auto text-left text-sm">
                ...
                <tbody>
                  {credentials.map(c => (
                    <motion.tr key={c.id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.2}} className="border-b border-slate-700">
                      ...
                      <td className="px-2 py-1 space-x-2">
                        <button onClick={() => resetCredential(c.id)} className="bg-yellow-500 px-2 py-1 rounded hover:bg-yellow-400 text-black text-xs">Reset</button>
                        <button onClick={() => deleteCredential(c.id)} className="bg-red-600 px-2 py-1 rounded hover:bg-red-500 text-xs">Delete</button>
                      </td>
                    </motion.tr>
                  ))}
                </tbody>
        ...
        <section>
          ...
          {loadingTrans ? <p>Loading transactions...</p> :
            <div className="overflow-x-auto bg-slate-800 rounded-2xl p-4 shadow">
              <table className="w-full table-auto text-left text-sm">
                <thead className="border-b border-slate-600 text-slate-300">
                  <tr>
                ...
                <tbody>
                  {transactions.map(t => (
                    <motion.tr key={t.id} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{duration:0.2}} className="border-b border-slate-700">
                      ...
                      <td className="px-2 py-1">{t.assignedAt ? new Date(t.assignedAt.seconds*1000).toLocaleString() : "-"}</td>
                    </motion.tr>
                  ))}
                </tbody>
              </table>
